...
const createMagazineCoverFlow = ai.defineFlow(
  ...
  async ({ imageUrls, title }) => {
    ...
    const { media } = await ai.generate({
      ...
      prompt: [
        { text: `Create a visually stunning and modern magazine cover collage titled "${title}". Arrange the following product images in a creative and appealing layout. The title should be prominent and stylish. The overall feel should be clean, fashionable, and aspirational. Do not add any other text besides the title.` },
        ...mediaParts,
      ],
      config: {
